...
class DyReLU(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(DyReLU, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, x):
        ...
        # print('x:', x.shape)
        # print('coeffs:', coeffs.shape, self.channels, a1.shape, b1.shape)
...
class TaskDecomposition(nn.Module):
    ...
    def forward(self, feat, avg_feat=None):
        # 在拼接后的特征上应用通道注意力
        # 再通过reduction_conv将上述特征降维到256，用于输出分类和回归结果
        b, c, h, w = feat.shape
        if avg_feat is None:
        ...
        # torch.bmm(a,b)，计算两个tensor的矩阵乘法，若a为[b, h, w]，b为[b, w, m]，则输出维度为[b, h, m]
...
class TFHeadTaskAttnNoShare(ATSSHead):
    ...
    def __init__(self,
        ...
        super(TFHeadTaskAttnNoShare, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        # 定义3个堆叠的卷积层，分类分支
        self.inter_cls_convs = nn.ModuleList()
        ...
        # 定义3个堆叠的卷积层，回归分支
        self.inter_reg_convs = nn.ModuleList()
        ...
        # 定义3个堆叠的卷积层，用于同时处理分类和回归分支，暂时不引入DCN
        ...
        # 用于降维
        self.reduction_conv_cls = ConvModule(
            self.in_channels * self.stacked_cls_convs,
            self.feat_channels,
            1,
            stride=1,
            padding=0,
            ...
            norm_cfg=self.norm_cfg,
            bias=None)
        self.reduction_conv_reg = ConvModule(
            self.in_channels * self.stacked_reg_convs,
            self.feat_channels,
            1,
            stride=1,
            padding=0,
        ...
        self.cls_decomp = TaskDecomposition(self.feat_channels,
                                            self.stacked_cls_convs,
                                            self.stacked_cls_convs * 8,
                                            self.conv_cfg, self.norm_cfg)

        # reg的通道注意力 + 降维
        self.reg_decomp = TaskDecomposition(self.feat_channels,
                                            self.stacked_reg_convs,
                                            self.stacked_reg_convs * 8,
                                            self.conv_cfg, self.norm_cfg)

        # 定义task_attn_module
        self.task_attn_module = DyReLU(self.in_channels)
        ...
        # print('num_base_priors:', num_base_priors)
    ...
    def init_weights(self):
        ...
        for m in self.inter_cls_convs:
            normal_init(m.conv, std=0.01)
        for m in self.inter_reg_convs:
            normal_init(m.conv, std=0.01)
        ...
        normal_init(self.reduction_conv_cls, std=0.01)
        normal_init(self.reduction_conv_reg, std=0.01)
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            # 首次提取分类特征
            inter_cls_feats = []
            for inter_conv in self.inter_cls_convs:
                cls_feat_ = inter_conv(cls_feat_)
                inter_cls_feats.append(cls_feat_)
            cls_feats = torch.cat(inter_cls_feats, 1)

            # avg_cls_feat = F.adaptive_avg_pool2d(cls_feats, (1, 1))

            # 首次提取回归特征
            inter_reg_feats = []
            for inter_conv in self.inter_reg_convs:
                reg_feat_ = inter_conv(reg_feat_)
                inter_reg_feats.append(reg_feat_)
            reg_feats = torch.cat(inter_reg_feats, 1)


            # 执行通道注意力并降维，以获得后一个堆叠卷积的输入
            cls_feat = self.reduction_conv_cls(cls_feats)
            reg_feat = self.reduction_conv_reg(reg_feats)

            # 计算用于输出分类和定位的特征
            inter_cls_feats = []
            inter_reg_feats = []
            for inter_conv in self.inter_convs:
                cls_feat = inter_conv(cls_feat)
                inter_cls_feats.append(cls_feat)
                reg_feat = inter_conv(reg_feat)
                inter_reg_feats.append(reg_feat)

            cls_feat = torch.cat(inter_cls_feats, 1)
            avg_cls_feat = F.adaptive_avg_pool2d(cls_feat, (1, 1))
            reg_feat = torch.cat(inter_reg_feats, 1)
            avg_reg_feat = F.adaptive_avg_pool2d(reg_feat, (1, 1))

            cls_feat = self.task_attn_module(self.cls_decomp(cls_feat, avg_cls_feat))
            reg_feat = self.task_attn_module(self.reg_decomp(reg_feat, avg_reg_feat))


            # cls prediction and alignment
            # 在通道注意力输出的256维feature上执行初始分类预测
            cls_logits = self.tood_cls(cls_feat)
            # 在堆叠特征上生成分类预测的矫正值，输入为堆叠的矩阵
            # cls_prob = self.cls_prob_module(cls_feats)
            # 同时使用sigmoid对上述输出激活，并保证即使cls_logits和cls_prob很小时，也存在梯度
            cls_score = F.sigmoid(cls_logits)

            # reg prediction and alignment
            if self.anchor_type == 'anchor_free':
                # 解码过程，将预测值转化为box，并除以FPN特征对应的步长
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, label_weights,
        ...
        # loss_cls继承自QFL
        # 见config文件对loss_cls的定义
        # TOOD中，self.activated为True
    ...
    def _get_target_single(self,
        ...
        # torch.unique取出独立不重复的元素
        class_assigned_gt_inds = torch.unique(
            sampling_result.pos_assigned_gt_inds)

        # print('class_assigned_gt_inds:', class_assigned_gt_inds)
        ...
        for gt_inds in class_assigned_gt_inds:
            ...
            # print('assign_ious:', pos_ious, pos_ious.shape, gt_inds)
            # print('assign_metrics:', pos_alignment_metrics, pos_alignment_metrics.shape, gt_inds)

            pos_norm_alignment_metrics = pos_alignment_metrics / (
                pos_alignment_metrics.max() + 10e-8) * pos_ious.max()
            

            # print('pos_norm_alignment_metrics:', pos_norm_alignment_metrics, gt_inds)

            # print('pos_alignment_metrics:', pos_alignment_metrics, pos_alignment_metrics.max(), pos_ious.max())
            # print('sampling_result.pos_assigned_gt_inds:', sampling_result.pos_assigned_gt_inds.shape)
