...
class HieAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None, epoch=None):
        """Assign gt to bboxes.
        ...
        # 注意，此处self.assign_metric的模式为'kl'，其计算过程见metric_calculator.py
        overlaps = self.iou_calculator(gt_bboxes, bboxes, mode=self.assign_metric)
        bboxes2 = self.anchor_rescale(bboxes, self.ratio)
        overlaps2 = self.iou_calculator(gt_bboxes, bboxes2, mode=self.assign_metric)

        # print('overlaps.mean():', overlaps.mean(), overlaps2.mean())
    ...
    def assign_wrt_ranking(self,  overlaps, k, gt_labels=None):
        ...
        for i in range(num_gts):
            for j in range(k):
                ...
                # print('overlaps:', overlaps[i,:], overlaps[i,:].shape)
                # print('gt_max_overlaps:', gt_max_overlaps[i, j])
                # print(overlaps[i,:] == gt_max_overlaps[i,j])
                # print('k', k)
                # print(assigned_gt_inds[max_overlap_inds])
    ...
    # bboxes格式为（左上，右下）。若ratio=1，则返回的bbox编码不变
